...
1) O stub no cliente serializa os parâmetros em bytes, envia via rede e desserializa a resposta, fazendo a chamada remota parecer local. O skeleton no servidor recebe os bytes, desserializa, roteia para a função correta e serializa a resposta de volta. Sem esses componentes, o desenvolvedor teria que escrever manualmente toda a serialização, o framing TCP, o tratamento de erros de rede e o roteamento para cada método, para cada linguagem e cada serviço. O stub e o skeleton existem para abstrair essa complexidade e são gerados automaticamente pelo protoc a partir do .proto, garantindo que cliente e servidor usem exatamente o mesmo protocolo.
2) No RPC a interface é orientada a ações, na Tarefa 1, proxy.calcular("soma", 7, 3) expõe um procedimento sem identificar recurso ou intenção via protocolo. No REST a interface é uniforme, na Tarefa 3, POST /produtos usa o verbo HTTP para comunicar criação e a URI para identificar o recurso, sem precisar de documentação extra para inferir a intenção.
3) O Protobuf lida com adição de campos via numeração de campos. Cada campo no .proto tem um número único (resultado = 1, unidade = 2). Ao adicionar unidade: string, clientes antigos que não conhecem o campo 2 simplesmente o ignoram, a comunicação não quebra. Isso é chamado de compatibilidade retroativa.
No REST sem schema, adicionar um campo no JSON também não quebra clientes existentes, eles ignoram campos desconhecidos naturalmente. A diferença é que no Protobuf isso é garantido pela especificação, enquanto no REST depende de como o cliente foi implementado — um cliente que usa parsing estrito pode quebrar.
4) Para a API externa com parceiros, REST, pela facilidade de adoção sem geração de código, JSON legível por humanos e uso de HTTP padrão que qualquer linguagem suporta nativamente. Para os 10 microsserviços internos, gRPC pelo contrato explícito via .proto que detecta incompatibilidades em tempo de compilação, Protobuf binário que reduz payload e latência em alto volume de chamadas internas, e StatusCode padronizado que facilita tratamento de erros consistente entre serviços.
5) A transparência do RPC pode levar o desenvolvedor a tratar uma chamada remota como se fosse local, ignorando que a rede é não-confiável. Um exemplo concreto: chamar stub.calcular("soma", 7, 3) dentro de um loop de 1000 iterações sem considerar latência, localmente isso seria imperceptível, remotamente pode levar segundos ou falhar por timeout. Outro exemplo é não tratar grpc.RpcError, uma chamada local nunca falha por problema de rede, mas uma chamada remota pode falhar por timeout, serviço indisponível ou perda de conexão. A transparência oculta essa diferença e o desenvolvedor só descobre em produção.